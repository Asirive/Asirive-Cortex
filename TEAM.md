# Team — Asirive Cortex

**Tan Kah Kee Young Inventors' Award 2026 Finalist**  
Admiralty Secondary School, Singapore

---

## Development Team

### Haziq Shah Bin Huzaimi
**Role:** Lead Developer & Founder  
**GitHub:** [@IRSPlays](https://github.com/IRSPlays)  
**Responsibilities:**
- System architecture and core development
- YOLO safety layer integration
- Gemini Live API implementation
- Navigation engine development

### Muhammad Irfan Nuafal Bin Mohd Fauzan
**Role:** AI/Navigation Integration  
**Responsibilities:**
- Navigation algorithms and GPS integration
- LTA DataMall transit integration
- YOLOE adaptive detection
- User testing and validation

### Eryna Natasha Binte Said Bohari
**Role:** User Experience & Validation  
**Responsibilities:**
- User interface design
- SAVH partnership coordination
- User feedback integration
- Documentation and presentation

---

## Mentors

### Kenneth Phua Khiang Song
**Organization:** Admiralty Secondary School  
**Contact:** 94560259  
**Support Area:** Technical Guidance  
**Role:**
- Technical mentorship and architecture review
- Hardware integration guidance
- Project timeline management

### Nur Syazana Binte Rahsid
**Organization:** Admiralty Secondary School  
**Contact:** 96562625  
**Support Area:** Project Coordination  
**Role:**
- Project coordination and administration
- YIA submission preparation
- Team logistics and resource management

---

## Institution

**Admiralty Secondary School**  
Singapore

**Fields of Invention:**
- Electrical/Electronic
- Infocomm
- Health Care

---

## Acknowledgments

### Singapore Association for the Visually Handicapped (SAVH)
Special thanks to the Singapore Association for the Visually Handicapped for their invaluable partnership, real-world testing feedback, and validation of the Asirive Cortex prototype. Their insights have been instrumental in shaping the product's design and functionality.

### Open Source Community
This project builds upon the following open-source technologies:
- **Google Gemini 3.1 Flash Live API** — Multimodal AI reasoning
- **YOLO / YOLOE** — Real-time object detection
- **Picamera2 / libcamera** — Raspberry Pi camera interface
- **Raspberry Pi Foundation** — Hardware platform
- **Hailo AI** — Edge NPU acceleration

---

**© 2026 Asirive Cortex Team**  
*Built for independence. Powered by Gemini. Designed with SAVH.*
